...
function draw(userChoise, computerChoice) {
  ...
  const smallCompWord = "comp".fontsize(3).sub();
  const userChoice_div = document.getElementById(userChoise);
  result_p.innerHTML = `${convertToWord(userChoise)}${smallUserWord} equal ${convertToWord(computerChoice)}${smallCompWord}. It's a draw!"`;
  userChoice_div.classList.add('blue-glow');
  setTimeout(() => userChoice_div.classList.remove('blue-glow'), 2000);
}
